  ...
  for( j=0; j<ny; j++)
    {
      for( i=0; i<nx; i++)
        ...
        initial_velocity(counter, Lx, Ly); // Initial velocity  according to Taylor-Green Vortices
...
void navierStokes_modified(void){
  ...
  // this is a nv equations modified according to (adami,2012)

  int i, j, k;
  double psiij, vdw, Aij;
  // computing sound speed and pression
  eos();
  ...
  for( i=0; i<nFluid; i++ ){
    ...
    for( k=0; k<part[i].nNeighbors; k++ ){
      ...
      psiij = (part[i].p * part[j].rho + part[j].p * part[i].rho) / 
              (part[i].rho + part[j].rho);

      part[i].accel[X] = part[i].accel[X] -
                         (1/part[i].mass) * Aij * psiij* part[i].dWx[k];
      part[i].accel[Y] = part[i].accel[Y] - 
                         (1/part[i].mass) * Aij * psiij*part[i].dWy[k];
